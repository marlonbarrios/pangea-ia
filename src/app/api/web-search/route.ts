  ...
  try {
    ...
    try {
      const response = await fetch(searchUrl, {
        headers: {
          'User-Agent': 'Pangea_IA/1.0 (Educational/Research)',
        },
      });
      ...
      if (!searchResults.instant_answer && !searchResults.definition && searchResults.related_topics.length === 0) {
        ...
        try {
          ...
          const fallbackResponse = await fetch(fallbackUrl, {
            headers: {
              'User-Agent': 'Mozilla/5.0 (compatible; Pangea_IA/1.0)',
            },
          });
      ...
      return NextResponse.json({
        ...
        results: {
          ...
          suggestions: [
            ...
            "Pregunta sobre temas relacionados con el proyecto Pangea_IA"
          ]
        },
        timestamp: new Date().toISOString()
      });
